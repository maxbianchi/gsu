...
@section('content')
        ...
        <div class="border">
            <form method="GET" action="{{url('/gsu/caselle/search')}}" name="form_search">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2"><input type="text" value="{{Input::get('account')}}" id="account" name="account" placeholder="ACCOUNT"></div>
                    <div class="col-md-1">EMAIL</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('email')}}" id="email" name="email" placeholder="EMAIL"></div>
                    <div class="col-md-2">MEGABYTE</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('megabyte')}}" id="megabyte" name="megabyte" placeholder="MEGABYTE"></div>
                    <div class="col-md-3"></div>
                </div>
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr>
                <td>
                    <a class="btn btn-small edit" href="{{url('/gsu/caselle/edit')."?id=".$req['IDCASELLA']}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn btn-small edit delete" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDCASELLA'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="11"><a class="btn btn-small edit" href="{{url('/gsu/caselle/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/caselle/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
