...
@section('content')
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        <tr>
            ...
            <th class="col-sm-2">PAGINA</th>
            <th class="col-sm-2">SPAZIO WEB</th>
        </tr>
        </thead>
        ...
        @foreach($request as $req)
            <tr>
                <td>
                    <a class="btn btn-small edit" href="{{url('/gsu/mailarchive/edit')."?id=".$req['IDMAILARCHIVE']}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn btn-small edit delete" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDMAILARCHIVE'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td><a href="{{url($class['link'][$req['CANONE']])."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['IDMAILARCHIVE']}}">{{$req['MANUTENZIONE']}}</a></td>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="9"><a class="btn btn-small edit" href="{{url('/gsu/mailarchive/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
