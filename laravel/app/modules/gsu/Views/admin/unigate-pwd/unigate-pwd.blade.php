...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/unigate-pwd/search')}}" name="form_search">
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="stato_left btn-small edit" href="{{url('/gsu/unigate-pwd/edit')."?apparato_id=".$req['UNIGATE_ID']."&id=".$req['IDUNIGATEPWD']."&eliminati=".Input::get('eliminati')."&manutenzione=".$req['MANUTENZIONE']}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
        ...
        @endforeach
            ...
            <tfoot>
            <tr>
                <th colspan="8"><a class="btn btn-small edit" href="{{url('/gsu/unigate-pwd/edit')."?seriale=".Input::get('seriale')."&id=".(isset($req['UNIGATE_ID']) ? $req['UNIGATE_ID'] : Input::get('apparato_id') )."&apparato_id=".(isset($req['UNIGATE_ID']) ? $req['UNIGATE_ID'] : Input::get('apparato_id') )."&isnew=1&manutenzione=".(isset($req['MANUTENZIONE']) ? $req['MANUTENZIONE'] : "" )}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/unigate-pwd/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
