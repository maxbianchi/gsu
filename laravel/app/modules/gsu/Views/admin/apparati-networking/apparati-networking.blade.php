    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                ...
                <td><a href="{{url(isset($class['link'][$req['CANONE']]) ? '/gsu/apparati-networking' : $class['link'][$req['CANONE']] )."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['ID']."&eliminati=".Input::get('eliminati')}}">{{$req['CODICE_R'] or "AGGIUNTO"}}</a></td>
