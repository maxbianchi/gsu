...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/apparati-mobile/search')}}" name="form_search">
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-1">NMU / PART NUMBER</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('pn')}}" id="pn" name="pn" ></div>
                    <div class="col-md-2">IMEI</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('imei')}}" id="imei" name="imei" ></div>
                    <div class="col-md-3"></div>
                </div>
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        <tr>
            ...
            <th class="col-sm-1">ACQUISTO / NOLEGGIO</th>
            <th class="col-sm-4">MODELLO</th>
        </tr>
        </thead>
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="stato_left btn-small edit" href="{{url('/gsu/apparati-mobile/edit')."?id=".$req['ID']."&eliminati=".Input::get('eliminati')}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="stato_right btn-small edit delete" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['ID'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td><a href="{{url(isset($class['link'][$req['CANONE']]) ? '/gsu/apparati-mobile' : $class['link'][$req['CANONE']] )."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['ID']."&eliminati=".Input::get('eliminati')}}">{{$req['CODICE_R']}}</a></td>
        ...
        @endforeach
            ...
            <tfoot>
            <tr>
                <th colspan="9"><a class="btn btn-small edit" href="{{url('/gsu/apparati-mobile/edit')."?isnew=1&manutenzione=".(isset($req['MANUTENZIONE']) ? $req['MANUTENZIONE'] : "" )}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/apparati-mobile/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
