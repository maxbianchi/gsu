    ...
    <fieldset class="dettaglio_dati">
        ...
        <table border="0" class="tabella dataTable table table-striped table-bordered display no-footer detail">
            <form action="#" method="post" id="form">
                ...
                <tr>
                    <td>OPZIONE ROAMING</td>
                    <td>
                        <select name="opzroaming">
                            <option value=""  	{{isset($request['OPZROAMING']) && $request['OPZROAMING'] == "" ? 'selected="selected"' : ""  }}>- Selezionare un opzione roaming -</option>
                            @foreach($opz as $row)
                                <option value="{{$row['NOME_OPZIONE']}}" {{isset($request['OPZROAMING']) && $request['OPZROAMING'] == $row['NOME_OPZIONE'] ? 'selected="selected"' : ""  }}>{{$row['NOME_OPZIONE']}}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#btn_salva").click(function(){
                         $.post( "{{url('/gsu/sim-roaming/save')}}", $("form#form").serialize())
                                .done(function( data ) {
                                    $('#msg').modal('show');
                                    $("#btn_salva").hide();
                                });
