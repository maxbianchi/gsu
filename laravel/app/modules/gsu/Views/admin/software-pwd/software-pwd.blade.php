...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/software-pwd/search')}}" name="form_search">
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="stato_left btn-small edit" href="{{url('/gsu/software-pwd/edit')."?apparato_id=".$req['IDSOFTWARE']."&id=".$req['IDSOFTWARE']."&eliminati=".Input::get('eliminati')."&manutenzione=".$req['MANUTENZIONE']}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="stato_right btn-small edit delete" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDPASSWORD'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                ...
                <td>{{$req['PASSWORD'] or ""}}</td>
                <td>{{$req['PWDPRIVILEGIATA'] or ""}}</td>
            </tr>
        @endforeach
        </tbody>
            ...
            <tfoot>
            <tr>
                <th colspan="7"><a class="btn btn-small edit" href="{{url('/gsu/software-pwd/edit')."?seriale=".Input::get('seriale')."&id=".(isset($req['IDSOFTWARE']) ? $req['IDSOFTWARE'] : Input::get('apparato_id') )."&apparato_id=".(isset($req['IDSOFTWARE']) ? $req['IDSOFTWARE'] : Input::get('apparato_id') )."&isnew=1&manutenzione=".(isset($req['MANUTENZIONE']) ? $req['MANUTENZIONE'] : "" )}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/software-pwd/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
