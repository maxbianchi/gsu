...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/unigate-numeri/search')}}" name="form_search">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-md-1">NUMERO TELEFONICO</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('numero_telefonico')}}" id="numero_telefonico" name="numero_telefonico" ></div>
                </div>
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="btn-small edit stato_left" href="{{url('/gsu/unigate-numeri/edit')."?id=".$req['IDUNIGATE_NUMERI']."&eliminati=".Input::get('eliminati')}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn-small edit delete stato_right" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDUNIGATE_NUMERI'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td>
                ...
                <td><a href="{{url($class['link'][$req['CANONE']])."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['IDUNIGATE_NUMERI']."&eliminati=".Input::get('eliminati')}}">{{$req['MANUTENZIONE']}}</a></td>
                ...
                <td>{{$req['NUMERO_TELEFONICO']}}</td>
                <td>{{$req['NUMERO_AGGIUNTIVO']}}</td>
            </tr>
        @endforeach
        </tbody>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="10"><a class="btn btn-small edit" href="{{url('/gsu/unigate-numeri/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/unigate-numeri/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
