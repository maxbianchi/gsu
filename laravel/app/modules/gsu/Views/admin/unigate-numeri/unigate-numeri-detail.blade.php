        ...
        <table border="0" class="tabella dataTable table table-striped table-bordered display no-footer detail">
            <form action="#" method="post" id="form">
                ...
                <tr>
                    ...
                    <td><input type="text" name="numero_telefonico" value="{{$request['NUMERO_TELEFONICO'] or ""}}"></td>
                </tr>
                <tr>
                    <td>NUMERO AGGIUNTIVO</td>
                    <td><input type="text" name="numero_aggiuntivo" value="{{$request['NUMERO_AGGIUNTIVO'] or ""}}"></td>
                </tr>

                <tr>
                    <td colspan="4" style="padding-top:20px;">
                        <input type="hidden" id="manutenzione" name="manutenzione" value="{{$request['MANUTENZIONE'] or Input::get("manutenzione")}}">
                        <input type="hidden" id="id_tbl" name="id_tbl" value="{{$request['IDUNIGATE_NUMERI'] or ""}}">
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#btn_salva").click(function(){
                         $.post( "{{url('/gsu/unigate-numeri/save')}}", $("form#form").serialize())
                                .done(function( data ) {
                                    $('#msg').modal('show');
                                    $("#btn_salva").hide();
                                });
