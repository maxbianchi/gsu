...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/novirusnospam/search')}}" name="form_search">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-md-1">DOMINIO</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('dominio')}}" id="dominio" name="dominio" placeholder="DOMINIO"></div>
                </div>
                <div class="row">
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr>
                <td>
                    <a class="btn btn-small edit" href="{{url('/gsu/novirusnospam/edit')."?id=".$req['IDNOVIRUSNOSPAM']}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn btn-small edit delete" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDNOVIRUSNOSPAM'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td><a href="{{url($class['link'][$req['CANONE']])."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['IDNOVIRUSNOSPAM']}}">{{$req['MANUTENZIONE']}}</a></td>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="8"><a class="btn btn-small edit" href="{{url('/gsu/novirusnospam/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/novirusnospam/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
