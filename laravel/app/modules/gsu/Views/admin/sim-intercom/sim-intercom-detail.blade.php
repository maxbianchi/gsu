    ...
    <fieldset class="dettaglio_dati">
        ...
        <table border="0" class="tabella dataTable table table-striped table-bordered display no-footer detail">
            <form action="#" method="post" id="form">
                ...
                <tr>
                    <td>OPZIONE INTERCOM</td>
                    <td>
                        <select name="opzintercom">
                            <option value=""  	{{isset($request['OPZINTERCOM']) && $request['OPZINTERCOM'] == "" ? 'selected="selected"' : ""  }}>- Selezionare un opzione roaming -</option>
                            @foreach($opz as $row)
                                <option value="{{$row['NOME_OPZIONE']}}" {{isset($request['OPZINTERCOM']) && $request['OPZINTERCOM'] == $row['NOME_OPZIONE'] ? 'selected="selected"' : ""  }}>{{$row['NOME_OPZIONE']}}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#btn_salva").click(function(){
                         $.post( "{{url('/gsu/sim-intercom/save')}}", $("form#form").serialize())
                                .done(function( data ) {
                                    $('#msg').modal('show');
                                    $("#btn_salva").hide();
                                });
