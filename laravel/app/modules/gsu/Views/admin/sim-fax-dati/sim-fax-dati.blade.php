...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/sim-fax-dati/search')}}" name="form_search">
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-1">N° FAX</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('noriginale')}}" id="noriginale" name="noriginale" ></div>
                    <div class="col-md-2">N° DATI</div>
                    <div class="col-md-2"><input type="text" value="{{Input::get('nextension')}}" id="nextension" name="nextension" ></div>
                    <div class="col-md-3"></div>
                </div>
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="btn-small edit stato_left" href="{{url('/gsu/sim-fax-dati/edit')."?id=".$req['IDSIM']."&eliminati=".Input::get('eliminati')}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn-small edit delete stato_right" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDSIM'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td>
                ...
                <td>{{$req['NTELEFONO']}}</td>
                <td>{{$req['NORIGINALE']}}</td>
                <td>{{$req['NEXTENSION']}}</td>
            </tr>
        @endforeach
        </tbody>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="11"><a class="btn btn-small edit" href="{{url('/gsu/sim-fax-dati/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/sim-fax-dati/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
