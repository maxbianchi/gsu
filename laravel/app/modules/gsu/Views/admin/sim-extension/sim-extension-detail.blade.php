    ...
    <fieldset class="dettaglio_dati">
        ...
        <table border="0" class="tabella dataTable table table-striped table-bordered display no-footer detail">
            <form action="#" method="post" id="form">
                ...
                <tr>
                    <td>N° AGGIUNTIVO</td>
                    <td><input type="text" name="noriginale" value="{{$request['NORIGINALE'] or ""}}"></td>
                </tr>
                <tr>
                    <td>PIANO TARIFFARIO</td>
                    <td>
                        <select name="pianotariffario">
                            <option value=""  	{{isset($request['PIANOTARIFFARIO']) && $request['PIANOTARIFFARIO'] == "" ? 'selected="selected"' : ""  }}>- Selezionare piano tariffario -</option>
                            @foreach($pianotariffario as $row)
                                <option value="{{$row['NOME_PIANO']}}" {{isset($request['PIANOTARIFFARIO']) && $request['PIANOTARIFFARIO'] == $row['NOME_PIANO'] ? 'selected="selected"' : ""  }}>{{$row['NOME_PIANO']}}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#btn_salva").click(function(){
                         $.post( "{{url('/gsu/sim-extension/save')}}", $("form#form").serialize())
                                .done(function( data ) {
                                    $('#msg').modal('show');
                                    $("#btn_salva").hide();
                                });
