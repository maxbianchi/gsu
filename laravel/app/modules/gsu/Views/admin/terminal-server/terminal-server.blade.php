...
@section('content')
    <div class="container-fluid">
        <div class="border">
            <form method="GET" action="{{url('/gsu/terminal-server/search')}}" name="form_search">
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2"><input type="text" value="{{Input::get('nome_host')}}" id="nome_host" name="nome_host" ></div>
    ...
    <table id="main" class="table table-striped table-bordered display" cellspacing="0" width="100%" style="display:none;">
        ...
        @foreach($request as $req)
            <tr class="{{$class[$req['MANUTENZIONE']]['GSU']["ELIMINATO"]}}">
                <td>
                    <a class="btn-small edit stato_left" href="{{url('/gsu/terminal-server/edit')."?id=".$req['IDTERMINAL_SERVER']."&eliminati=".Input::get('eliminati')}}" title="EDIT"><i class="glyphicon glyphicon-pencil"></i> </a>
                    <a class="btn-small edit delete stato_right" href="javascript:void(0);" data-toggle="modal" title="DELETE" manutenzione="{{$req['MANUTENZIONE'] or ""}}" delete-id="{{$req['IDTERMINAL_SERVER'] or ""}}"><i class="glyphicon glyphicon-trash"></i> </a>
                </td>
                <td>
                ...
                <td><a href="{{url($class['link'][$req['CANONE']])."/show?manutenzione=".$req['MANUTENZIONE']."&id=".$req['IDTERMINAL_SERVER']."&eliminati=".Input::get('eliminati')}}">{{$req['MANUTENZIONE']}}</a></td>
        ...
        @if(Input::get('add') == 1)
            <tfoot>
            <tr>
                <th colspan="11"><a class="btn btn-small edit" href="{{url('/gsu/terminal-server/edit')."?isnew=1&manutenzione=".$req['MANUTENZIONE']}}" title="ADD NEW"><i class="glyphicon glyphicon-plus"></i>&nbsp; ADD NEW </a></th>
            </tr>
            </tfoot>
        @endif
    </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btn_elimina").click(function(){
                $.get( "{{url('/gsu/terminal-server/delete')}}", { id: id_elimina, manutenzione: manutenzione } )
                        .done(function( data ) {
                            $("#delete").modal('toggle');
                            $("#cerca").trigger("click");
                        });
