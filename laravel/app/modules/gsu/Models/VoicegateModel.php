...
class VoicegateModel extends Model {
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND VOICEGATE.IDVOICEGATE = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($apparato))
            $sql .= " AND VOICEGATE.APPARATO like '%$apparato%'";
        if(!empty($sn))
            $sql .= " AND VOICEGATE.SN like '%$sn%'";
        if(!empty($numero))
            $sql .= " AND VOICEGATE.NUMERO like '%$numero%'";


        if(!empty($eliminati))
            $sql .= " AND VOICEGATE.ELIMINATO = 1";
        else
            $sql .= " AND VOICEGATE.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.VOICEGATE SET ELIMINATO=1 WHERE IDVOICEGATE='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.VOICEGATE (CODICE_R,APPARATO,SN,NUMERO,ELIMINATO) VALUES ('$manutenzione','$apparato','$sn','$numero',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.VOICEGATE SET Codice_R='$manutenzione',APPARATO='$apparato',SN='$sn',NUMERO='$numero',ELIMINATO=$eliminato WHERE IDVOICEGATE=$id");
