...
class VideoconferenceModel extends Model {
    ...
    public function getAllRequest(){
        ...
        SELECT
			...
			VIDEOCONFERENCE.IDVIDEOCONFERENCE,
			VIDEOCONFERENCE.CODICE_R,
			VIDEOCONFERENCE.DATA_R,
			VIDEOCONFERENCE.NUTENTI,
			VIDEOCONFERENCE.SPAZIOWEB,
			VIDEOCONFERENCE.UTENTE,
			VIDEOCONFERENCE.PASSWORD,
			VIDEOCONFERENCE.ELIMINATO
			FROM		gsu.dbo.VIDEOCONFERENCE
			RIGHT OUTER JOIN			UNIWEB.dbo.AOF70	richieste	ON VIDEOCONFERENCE.codice_r				= richieste.MANUTENZIONE
            ...
            WHERE VIDEOCONFERENCE.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND VIDEOCONFERENCE.IDVIDEOCONFERENCE = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($eliminati))
            $sql .= " AND VIDEOCONFERENCE.ELIMINATO = 1";
        else
            $sql .= " AND VIDEOCONFERENCE.ELIMINATO = 0";
    ...
    public function saveData(){
        ...
        $numeroutenti = Input::get('nutenti');
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.VIDEOCONFERENCE (Codice_R,NUTENTI,SPAZIOWEB,UTENTE,PASSWORD ELIMINATO) VALUES ('$manutenzione','$numeroutenti','$spazioweb','$utente','$password',$eliminato)");
                $sql = "SELECT * FROM gsu.dbo.RICHIESTE_EVASE WHERE CODICE_R = '$manutenzione'";
                $richieste_evase = DB::select($sql);
                if(count($richieste_evase) > 0) {
                    $richieste_evase = $richieste_evase[0];
                ...
                DB::update("UPDATE gsu.dbo.VIDEOCONFERENCE SET Codice_R='$manutenzione', NUTENTI='$numeroutenti',SPAZIOWEB='$spazioweb',UTENTE='$utente',PASSWORD='$password', ELIMINATO=$eliminato WHERE IDVIDEOCONFERENCE=$id");
    ...
    public function checkAddNew(){
        $model = new VideoconferenceModel();
