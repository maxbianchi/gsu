...
class GsuModel extends Model {
    ...
    public function getAllRequest(){
        ...
        CASE WHEN (SUBSTRING(RICHIESTE.OGGETTO,1,8) = 'CAN-SW06') THEN (SOFTWARE.QUANTITA) ELSE (ISNULL(RICHIESTE_EVASE.QUANTITA, 0)) END QTAGSU,
        --ISNULL(RICHIESTE_EVASE.QUANTITA, 0) AS QTAGSU,
        RICHIESTE.NRCONTRATTO,
        CONVERT(VARCHAR(10),RICHIESTE.DATASCADENZA,105) DATASCADENZA,
        CONVERT(VARCHAR(10),RICHIESTE.DATAAPERTURA,105) DATAAPERTURA,
        CASE WHEN (RICHIESTE.DATADISATTIVAZIONE > '1990-01-01') THEN CONVERT(VARCHAR(10),RICHIESTE.DATADISATTIVAZIONE,105) ELSE '' END  AS DATADISATTIVAZIONE,
        ISNULL(REPLACE(ROUND(RICHIESTE.VALOREUNITARIO, 2, 1),'.',','), 0) VALOREUNITARIO,
        ISNULL(REPLACE(ROUND(RICHIESTE.FORMULASCONTO, 2, 1),'.',','), 0) FORMULASCONTO,
        ISNULL(REPLACE(ROUND(RICHIESTE.TOTRIGANETTO, 2, 1),'.',','), 0) TOTRIGANETTO,
        ISNULL(REPLACE(ROUND(RICHIESTE.VALOREACQUISTO, 2, 1),'.',','), 0) VALOREACQUISTO
        FROM
        ...
        LEFT OUTER JOIN gsu.dbo.SOFTWARE ON SOFTWARE.codice_r = richieste.MANUTENZIONE
