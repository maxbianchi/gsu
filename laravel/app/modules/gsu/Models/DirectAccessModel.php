...
class DirectAccessModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			DIRECTACCESS.TGU,
			DIRECTACCESS.IP_STATICO_ROUTER,
			...
			RIGHT OUTER JOIN UNIWEB.dbo.AOF70 richieste ON DIRECTACCESS.codice_r = richieste.MANUTENZIONE
...
EOF;
    ...
    public function getFilteredRequest(){
        ...
        $sql = <<<EOF
			...
			DIRECTACCESS.GATEWAY_INTERFACCIA_LAN,
			DIRECTACCESS.NUMERO_IP_STATICI,
			DIRECTACCESS.MODALITA,
			DIRECTACCESS.DLCI,
            DIRECTACCESS.NUMERO_TELEFONO,
			DIRECTACCESS.LMI_TYPE,
			DIRECTACCESS.IP_STATICI_SUBNET,
            DIRECTACCESS.DNS_PRIMARIO,
			DIRECTACCESS.DNS_SECONDARIO,
			DIRECTACCESS.N_VERDE,
			DIRECTACCESS.VPI,
			DIRECTACCESS.VCI,
            DIRECTACCESS.INSTALLAZIONE_MODEM,
			DIRECTACCESS.INCAPSULAMENTO,
            DIRECTACCESS.IPSUB,
			DIRECTACCESS.WANSUB,
			DIRECTACCESS.LANSUB,
			DIRECTACCESS.RUTSUB,
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND DIRECTACCESS.IDDIRECTACCESS = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($tgu))
            $sql .= " AND DIRECTACCESS.TGU like '%$tgu%'";
        if(!empty($ip_router))
            $sql .= " AND DIRECTACCESS.IP_STATICO_ROUTER like '%$ip_router%'";
