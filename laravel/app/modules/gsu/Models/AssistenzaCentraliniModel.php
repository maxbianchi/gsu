...
class AssistenzaCentraliniModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			ASSISTENZACENTRALINI.IDASSISTENZACENTRALINO,
			ASSISTENZACENTRALINI.SERIALE,
			ASSISTENZACENTRALINI.CODICE_R,
			ASSISTENZACENTRALINI.ELIMINATO
			FROM gsu.dbo.ASSISTENZACENTRALINI
			RIGHT OUTER JOIN UNIWEB.dbo.AOF70 richieste ON ASSISTENZACENTRALINI.codice_r = richieste.MANUTENZIONE
            ...
            WHERE ASSISTENZACENTRALINI.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND ASSISTENZACENTRALINI.IDASSISTENZACENTRALINO = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($seriale))
            $sql .= " AND ASSISTENZACENTRALINI.SERIALE like '%$seriale%'";


        if(!empty($eliminati))
            $sql .= " AND ASSISTENZACENTRALINI.ELIMINATO = 1";
        else
            $sql .= " AND ASSISTENZACENTRALINI.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.ASSISTENZACENTRALINI SET ELIMINATO=1 WHERE IDASSISTENZACENTRALINO='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.ASSISTENZACENTRALINI (CODICE_R,SERIALE, ELIMINATO) VALUES ('$manutenzione','$seriale',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.ASSISTENZACENTRALINI SET Codice_R='$manutenzione',SERIALE='$seriale',ELIMINATO=$eliminato WHERE IDASSISTENZACENTRALINO=$id");
    ...
    public function checkAddNew(){
        $model = new AssistenzaCentraliniModel();
