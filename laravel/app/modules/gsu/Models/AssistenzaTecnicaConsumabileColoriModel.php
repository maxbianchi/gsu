...
class AssistenzaTecnicaConsumabileColoriModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ID,
			ASSISTENZA_TECNICA_CONSUMABILE_COLORI.SN,
			ASSISTENZA_TECNICA_CONSUMABILE_COLORI.CODICE_R,
			ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ELIMINATO
			FROM gsu.dbo.ASSISTENZA_TECNICA_CONSUMABILE_COLORI
			RIGHT OUTER JOIN UNIWEB.dbo.AOF70 richieste ON ASSISTENZA_TECNICA_CONSUMABILE_COLORI.codice_r = richieste.MANUTENZIONE
            ...
            WHERE ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ID = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($seriale))
            $sql .= " AND ASSISTENZA_TECNICA_CONSUMABILE_COLORI.SN like '%$seriale%'";


        if(!empty($eliminati))
            $sql .= " AND ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ELIMINATO = 1";
        else
            $sql .= " AND ASSISTENZA_TECNICA_CONSUMABILE_COLORI.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.ASSISTENZA_TECNICA_CONSUMABILE_COLORI SET ELIMINATO=1 WHERE ID='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.ASSISTENZA_TECNICA_CONSUMABILE_COLORI (CODICE_R,SN, ELIMINATO) VALUES ('$manutenzione','$seriale',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.ASSISTENZA_TECNICA_CONSUMABILE_COLORI SET Codice_R='$manutenzione',SN='$seriale',ELIMINATO=$eliminato WHERE ID=$id");
