...
class UtenteAggiuntivoTSModel extends Model {
    ...
    public function getAllRequest(){
        ...
        SELECT
            ...
            UTENTE_AGGIUNTIVO_TS.IDUTENTE,
            UTENTE_AGGIUNTIVO_TS.CODICE_R,
            UTENTE_AGGIUNTIVO_TS.NOME,
            UTENTE_AGGIUNTIVO_TS.COGNOME,
            UTENTE_AGGIUNTIVO_TS.UTENTE,
            UTENTE_AGGIUNTIVO_TS.PWD,
            UTENTE_AGGIUNTIVO_TS.NOTE,
            UTENTE_AGGIUNTIVO_TS.ELIMINATO
			FROM		gsu.dbo.UTENTE_AGGIUNTIVO_TS
			RIGHT OUTER JOIN			UNIWEB.dbo.AOF70	richieste	ON UTENTE_AGGIUNTIVO_TS.codice_r				= richieste.MANUTENZIONE
            ...
            WHERE UTENTE_AGGIUNTIVO_TS.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND UTENTE_AGGIUNTIVO_TS.IDUTENTE = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($nome))
            $sql .= " AND UTENTE_AGGIUNTIVO_TS.NOME like '%$nome%'";
        if(!empty($cognome))
            $sql .= " AND UTENTE_AGGIUNTIVO_TS.COGNOME like '%$cognome%'";

        if(!empty($eliminati))
            $sql .= " AND UTENTE_AGGIUNTIVO_TS.ELIMINATO = 1";
        else
            $sql .= " AND UTENTE_AGGIUNTIVO_TS.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.UTENTE_AGGIUNTIVO_TS SET ELIMINATO=1 WHERE IDUTENTE='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.UTENTE_AGGIUNTIVO_TS (Codice_R, NOME, COGNOME,UTENTE,PWD,NOTE,ELIMINATO) VALUES ('$manutenzione','$nome','$cognome','$utente','$pwd','$note',$eliminato)");
                $sql = "SELECT * FROM gsu.dbo.RICHIESTE_EVASE WHERE CODICE_R = '$manutenzione'";
                $richieste_evase = DB::select($sql);
                if(count($richieste_evase) > 0) {
                    $richieste_evase = $richieste_evase[0];
                ...
                DB::update("UPDATE gsu.dbo.UTENTE_AGGIUNTIVO_TS SET Codice_R='$manutenzione',NOME='$nome', COGNOME='$cognome',UTENTE='$utente',PWD='$pwd',NOTE='$note',ELIMINATO=$eliminato  WHERE IDUTENTE=$id");
    ...
    public function checkAddNew(){
        $model = new UtenteAggiuntivoTSModel();
