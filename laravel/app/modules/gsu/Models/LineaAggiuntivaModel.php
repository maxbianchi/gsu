...
class LineaAggiuntivaModel extends Model {
    ...
    public function getAllRequest(){
        ...
        SELECT
			...
			RIGHT OUTER JOIN			UNIWEB.dbo.AOF70	richieste	ON LINEA_AGGIUNTIVA.codice_r = richieste.MANUTENZIONE
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND LINEA_AGGIUNTIVA.IDLINEA = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
