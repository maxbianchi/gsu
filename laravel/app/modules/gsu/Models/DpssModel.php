...
EOF;
    ...
    public function getFilteredRequest(){
        ...
        $n_postazioni = Input::get('n_postazioni');
...
EOF;
        ...
        if(!empty($n_postazioni))
            $sql .= " AND DPSS.N_POSTAZIONI like '%$n_postazioni%'";



        if(!empty($eliminati))
            $sql .= " AND DPSS.ELIMINATO = 1";
        else
            $sql .= " AND DPSS.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.DPSS SET ELIMINATO=1 WHERE IDDPSS='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.DPSS (CODICE_R,N_POSTAZIONI,ELIMINATO) VALUES ('$manutenzione','$n_postazioni',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.DPSS SET Codice_R='$manutenzione',N_POSTAZIONI='$n_postazioni',ELIMINATO=$eliminato WHERE IDDPSS=$id");
