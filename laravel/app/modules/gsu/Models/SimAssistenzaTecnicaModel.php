...
class SimAssistenzaTecnicaModel extends Model {
...
EOF;
        ...
        if(!empty($imei))
            $sql .= " AND SIM.SERIALE_TEL like '%$imei%'";
        if(!empty($opzintercom))
            $sql .= " AND SIM.OPZINTERCOM like '%$opzintercom%'";
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.SIM (CODICE_R,NTELEFONO,CCID,TIPOSIM,TEC_GSM,TEC_UMTS,TEC_EDGE,TGC,CATCHIAMATE,PROMOVOCE,NBREVE,RESTRIZIONI,SERIALE_TEL, ELIMINATO) VALUES ('$manutenzione','$ntelefono','$ccid','$tiposim','$tec_gsm','$tec_umts','$tec_edge','$tgc','$catchiamate','$promovoce','$nbreve','$restrisioni','$imei',$eliminato)");
                ...
                $sql = "UPDATE gsu.dbo.SIM SET Codice_R='$manutenzione',NTELEFONO='$ntelefono',SERIALE_TEL='$imei',CCID='$ccid',TIPOSIM='$tiposim',TEC_GSM='$tec_gsm',TEC_UMTS='$tec_umts',TEC_EDGE='$tec_edge',TGC='$tgc',CATCHIAMATE='$catchiamate',PROMOVOCE='$promovoce',NBREVE='$nbreve',RESTRIZIONI='$restrisioni',ELIMINATO=$eliminato WHERE IDSIM=$id";
                DB::update($sql);
    ...
    public function getOpzioniIntercom(){
        $sql = "SELECT * FROM SIM_OPZIONI_INTERCOM ORDER BY NOME_OPZIONE ASC";
        $opz  = DB::select($sql);
        return $opz;
    }
