...
class SimFiltroAccessiModel extends Model {
...
EOF;
    ...
    public function getFilteredRequest(){
        ...
        $filtroaccessi = Input::get('filtroaccessi');
...
EOF;
        ...
        if(!empty($filtroaccessi))
            $sql .= " AND SIM.FILTROACCESSI like '%$filtroaccessi%'";
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.SIM (CODICE_R,NTELEFONO,CCID,TIPOSIM,TEC_GSM,TEC_UMTS,TEC_EDGE,TGC,CATCHIAMATE,PROMOVOCE,NBREVE,RESTRIZIONI,FILTROACCESSI, ELIMINATO) VALUES ('$manutenzione','$ntelefono','$ccid','$tiposim','$tec_gsm','$tec_umts','$tec_edge','$tgc','$catchiamate','$promovoce','$nbreve','$restrisioni','$filtroaccessi',$eliminato)");
                ...
                $sql = "UPDATE gsu.dbo.SIM SET Codice_R='$manutenzione',FILTROACCESSI='$filtroaccessi',SERIALE_TEL='$imei',CCID='$ccid',TIPOSIM='$tiposim',TEC_GSM='$tec_gsm',TEC_UMTS='$tec_umts',TEC_EDGE='$tec_edge',TGC='$tgc',CATCHIAMATE='$catchiamate',PROMOVOCE='$promovoce',NBREVE='$nbreve',RESTRIZIONI='$restrisioni',ELIMINATO=$eliminato WHERE IDSIM=$id";
                DB::update($sql);
    ...
    public function getFiltroAccessi(){
        $sql = "SELECT * FROM SIM_FILTRO_ACCESSI ORDER BY NOME_FILTRO ASC";
        $opz  = DB::select($sql);
        return $opz;
    }
