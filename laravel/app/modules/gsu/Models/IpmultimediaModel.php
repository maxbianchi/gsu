...
class IpmultimediaModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			IPMULTIMEDIA.IDIPMULTIMEDIA,
			IPMULTIMEDIA.CODICE_R,
            IPMULTIMEDIA.APPARATO,
            IPMULTIMEDIA.SN,
            IPMULTIMEDIA.CANALI,
            ...
            WHERE IPMULTIMEDIA.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND IPMULTIMEDIA.IDIPMULTIMEDIA = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($apparato))
            $sql .= " AND IPMULTIMEDIA.APPARATO like '%$apparato%'";
        if(!empty($sn))
            $sql .= " AND IPMULTIMEDIA.SN like '%$sn%'";
        if(!empty($canali))
            $sql .= " AND IPMULTIMEDIA.CANALI like '%$canali%'";


        if(!empty($eliminati))
            $sql .= " AND IPMULTIMEDIA.ELIMINATO = 1";
        else
            $sql .= " AND IPMULTIMEDIA.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.IPMULTIMEDIA SET ELIMINATO=1 WHERE IDIPMULTIMEDIA='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.IPMULTIMEDIA (CODICE_R,APPARATO,SN,CANALI,ELIMINATO) VALUES ('$manutenzione','$apparato','$sn','$canali',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.IPMULTIMEDIA SET Codice_R='$manutenzione',APPARATO='$apparato',SN='$sn',CANALI='$canali',ELIMINATO=$eliminato WHERE IDIPMULTIMEDIA=$id");
