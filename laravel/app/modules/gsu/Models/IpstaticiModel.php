...
class IpstaticiModel extends Model {
...
EOF;
    ...
    public function getFilteredRequest(){
        ...
        $tgu_linea = Input::get('tgu_linea');
        $numero_ip = Input::get('numero_ip');
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND IPSTATICI.IDIPSTATICO = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($tgu_linea))
            $sql .= " AND IPSTATICI.TGU_LINEA like '%$tgu_linea%'";
        if(!empty($numero_ip))
            $sql .= " AND IPSTATICI.NUM_IP like '%$numero_ip%'";

        if(!empty($eliminati))
            $sql .= " AND IPSTATICI.ELIMINATO = 1";
        else
            $sql .= " AND IPSTATICI.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.IPSTATICI SET ELIMINATO=1 WHERE IDIPSTATICO='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.IPSTATICI (Codice_R, TGU_LINEA, NUM_IP,ELIMINATO) VALUES ('$manutenzione','$tgu_linea','$numero_ip',$eliminato)");
                $sql = "SELECT * FROM gsu.dbo.RICHIESTE_EVASE WHERE CODICE_R = '$manutenzione'";
                $richieste_evase = DB::select($sql);
                if(count($richieste_evase) > 0) {
                    $richieste_evase = $richieste_evase[0];
                ...
                DB::update("UPDATE gsu.dbo.IPSTATICI SET Codice_R='$manutenzione',TGU_LINEA='$tgu_linea', NUM_IP='$numero_ip',ELIMINATO=$eliminato  WHERE IDIPSTATICO=$id");
