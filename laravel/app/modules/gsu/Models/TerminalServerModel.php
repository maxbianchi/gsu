...
class TerminalServerModel extends Model {
    ...
    public function getAllRequest(){
        ...
        SELECT
            ...
            TERMINAL_SERVER.IDTERMINAL_SERVER,
            TERMINAL_SERVER.SERIALE,
            TERMINAL_SERVER.CODICE_R,
            TERMINAL_SERVER.NOMEPC,
            TERMINAL_SERVER.IP,
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND TERMINAL_SERVER.IDTERMINAL_SERVER = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($ip))
            $sql .= " AND IPSTATICI.IP like '%$ip%'";
        if(!empty($nome_host))
            $sql .= " AND IPSTATICI.NOMEPC like '%$nome_host%'";

        if(!empty($eliminati))
            $sql .= " AND TERMINAL_SERVER.ELIMINATO = 1";
        else
            $sql .= " AND TERMINAL_SERVER.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.TERMINAL_SERVER SET ELIMINATO=1 WHERE IDTERMINAL_SERVER='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.TERMINAL_SERVER (Codice_R, SERIALE, NOMEPC,IP,NOTE,ELIMINATO) VALUES ('$manutenzione','$seriale','$nome_host','$ip','$note',$eliminato)");
                $sql = "SELECT * FROM gsu.dbo.RICHIESTE_EVASE WHERE CODICE_R = '$manutenzione'";
                $richieste_evase = DB::select($sql);
                if(count($richieste_evase) > 0) {
                    $richieste_evase = $richieste_evase[0];
                ...
                DB::update("UPDATE gsu.dbo.TERMINAL_SERVER SET Codice_R='$manutenzione',SERIALE='$seriale', NOMEPC='$nome_host',IP='$ip',NOTE='$note',ELIMINATO=$eliminato  WHERE IDTERMINAL_SERVER=$id");
