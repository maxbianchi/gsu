...
class AssistenzaTecnicaHWModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			TELEASSISTENZA.IDTELEASSISTENZA,
			TELEASSISTENZA.SERIALE,
			TELEASSISTENZA.NOMEPC,
			TELEASSISTENZA.OS,
			TELEASSISTENZA.ACCESSO,
            TELEASSISTENZA.IP,
            TELEASSISTENZA.NOTE,
			TELEASSISTENZA.IPPUBBLICO,
			TELEASSISTENZA.ELIMINATO
			FROM gsu.dbo.TELEASSISTENZA
			LEFT OUTER JOIN UNIWEB.dbo.AOF70 richieste ON TELEASSISTENZA.codice_r = richieste.MANUTENZIONE
            ...
            WHERE TELEASSISTENZA.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND TELEASSISTENZA.IDTELEASSISTENZA = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($nomepc))
            $sql .= " AND TELEASSISTENZA.NOMEPC like '%$nomepc%'";
        if(!empty($seriale))
            $sql .= " AND TELEASSISTENZA.SERIALE like '%$seriale%'";
        if(!empty($os))
            $sql .= " AND TELEASSISTENZA.OS like '%$os%'";
        if(!empty($accesso))
            $sql .= " AND TELEASSISTENZA.ACCESSO like '%$accesso%'";

        if(!empty($eliminati))
            $sql .= " AND TELEASSISTENZA.ELIMINATO = 1";
        else
            $sql .= " AND TELEASSISTENZA.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.TELEASSISTENZA SET ELIMINATO=1 WHERE IDTELEASSISTENZA='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        $ip = Input::get('ip');
        $note = Input::get('note');
        $ippubblico = Input::get('ippubblico');

        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.TELEASSISTENZA (CODICE_R,SERIALE,NOMEPC,OS,ACCESSO,IP,NOTE,IPPUBBLICO,ELIMINATO) VALUES ('$manutenzione','$seriale','$nomepc','$os','$accesso','$ip','$note','$ippubblico',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.TELEASSISTENZA SET Codice_R='$manutenzione',SERIALE='$seriale',NOMEPC='$nomepc',OS='$os',ACCESSO='$accesso',IP='$ip',NOTE='$note',IPPUBBLICO='$ippubblico', ELIMINATO=$eliminato WHERE IDTELEASSISTENZA=$id");
    ...
    public function checkAddNew(){
        $model = new AssistenzaTecnicaHWModel();
