...
class GestioneApparatiModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			GESTIONE_APPARATI.IDAPPARATO,
			GESTIONE_APPARATI.SERIALE,
			GESTIONE_APPARATI.CODICE_R,
			GESTIONE_APPARATI.SERIALE,
			GESTIONE_APPARATI.TOKEN,
			GESTIONE_APPARATI.SERVICE,
			GESTIONE_APPARATI.PRODUCT_CODE,
			GESTIONE_APPARATI.CONTRATTO,
			GESTIONE_APPARATI.ELIMINATO
			FROM gsu.dbo.GESTIONE_APPARATI
			RIGHT OUTER JOIN UNIWEB.dbo.AOF70 richieste ON GESTIONE_APPARATI.codice_r = richieste.MANUTENZIONE
            ...
            WHERE GESTIONE_APPARATI.ELIMINATO = 0
EOF;
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND GESTIONE_APPARATI.IDAPPARATO = '$id'";
        if(!empty($cliente))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA1.DESCRIZIONE)),'''','') like '%$cliente%'";
        if(!empty($cliente_finale))
            $sql .= " AND REPLACE(LTRIM(RTRIM(ANAGRAFICA2.DESCRIZIONE)),'''','') like '%$cliente_finale%'";
        ...
        if(!empty($seriale))
            $sql .= " AND GESTIONE_APPARATI.SERIALE like '%$seriale%'";
        if(!empty($token))
            $sql .= " AND GESTIONE_APPARATI.TOKEN like '%$token%'";
        if(!empty($service))
            $sql .= " AND GESTIONE_APPARATI.SERVICE like '%$service%'";
        if(!empty($product_code))
            $sql .= " AND GESTIONE_APPARATI.PRODUCT_CODE like '%$product_code%'";
        if(!empty($contratto))
            $sql .= " AND GESTIONE_APPARATI.CONTRATTO like '%$contratto%'";


        if(!empty($eliminati))
            $sql .= " AND GESTIONE_APPARATI.ELIMINATO = 1";
        else
            $sql .= " AND GESTIONE_APPARATI.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.GESTIONE_APPARATI SET ELIMINATO=1 WHERE IDAPPARATO='$id'";
            DB::update($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.GESTIONE_APPARATI (CODICE_R,SERIALE,TOKEN,SERVICE,PRODUCT_CODE,CONTRATTO ELIMINATO) VALUES ('$manutenzione','$seriale','$token','$service','$product_code','$contratto',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.GESTIONE_APPARATI SET Codice_R='$manutenzione',SERIALE='$seriale',TOKEN='$token',SERVICE='$service',PRODUCT_CODE='$product_code',CONTRATTO='$contratto',ELIMINATO=$eliminato WHERE IDAPPARATO=$id");
    ...
    public function checkAddNew(){
        $model = new GestioneApparatiModel();
