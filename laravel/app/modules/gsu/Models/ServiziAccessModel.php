...
class ServiziAccessModel extends Model {
    ...
    public function getAllRequest(){
        ...
        $sql = <<<EOF
			...
			SERVIZI_ACCESS.IDSERVIZIO,
			SERVIZI_ACCESS.CODICE_R,
            SERVIZI_ACCESS.TGU_PRIMARIA,
            ...
            WHERE SERVIZI_ACCESS.ELIMINATO = 0
EOF;
    ...
    public function getFilteredRequest(){
        ...
        $tgu_primaria = Input::get('tgu_primaria');
        $tgu_secondaria = Input::get('tgu_secondaria');
...
EOF;
        ...
        if(!empty($id))
            $sql .= " AND SERVIZI_ACCESS.IDSERVIZIO = '$id'";
        if(!empty($cliente))
            $sql .= " AND ANAGRAFICA1.DESCRIZIONE like '%$cliente%'";
        ...
        if(!empty($tgu_primaria))
            $sql .= " AND SERVIZI_ACCESS.TGU_PRIMARIA like '%$tgu_primaria%'";
        if(!empty($tgu_secondaria))
            $sql .= " AND SERVIZI_ACCESS.TGU_SECONDARIA like '%$tgu_secondaria%'";


        if(!empty($eliminati))
            $sql .= " AND SERVIZI_ACCESS.ELIMINATO = 1";
        else
            $sql .= " AND SERVIZI_ACCESS.ELIMINATO = 0";
    ...
    public function deleteByID(){
        ...
        $manutenzione = Input::get('manutenzione');
        if(!empty($id)) {
            $sql = "UPDATE gsu.dbo.SERVIZI_ACCESS SET ELIMINATO=1 WHERE IDSERVIZIO='$id'";
            DB::delete($sql);
    ...
    public function saveData(){
        ...
        try {
            if(empty($id)) {
                DB::insert("INSERT INTO gsu.dbo.SERVIZI_ACCESS (CODICE_R,TGU_PRIMARIA,TGU_SECONDARIA,ELIMINATO) VALUES ('$manutenzione','$tgu_primaria','$tgu_secondaria',$eliminato)");
                ...
                DB::update("UPDATE gsu.dbo.SERVIZI_ACCESS SET Codice_R='$manutenzione',TGU_PRIMARIA='$tgu_primaria',TGU_SECONDARIA='$tgu_secondaria',ELIMINATO=$eliminato WHERE IDSERVIZIO=$id");
