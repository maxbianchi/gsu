...
class ClientiModel extends Model {
   ...
   public function getAllStorico($idattivita){
       $sql = "SELECT *, CONVERT(VARCHAR(15),STORICO.DATA,105) DATACAMBIOSTATO, CONVERT(VARCHAR(15),STORICO.DATA,108) ORACAMBIOSTATO FROM TICKET.dbo.STORICO INNER JOIN TICKET.dbo.STATI ON TICKET.dbo.STORICO.IDSTATO = TICKET.dbo.STATI.IDSTATO WHERE TICKET.dbo.STORICO.IDATTIVITA='$idattivita' ORDER BY ID ASC";
       $request  = DB::select($sql);
       return $request;
   }

    public function getAttivita($idattivita){
        $sql = "SELECT *,CONVERT(VARCHAR(15),ATTIVITA.APERTAIL,105) APERTAIL,CONVERT(VARCHAR(15),ATTIVITA.APERTAIL,108) ORAAPERTAIL FROM TICKET.dbo.ATTIVITA WHERE IDATTIVITA='$idattivita'";
        $request  = DB::select($sql);
        return $request[0];
    }
