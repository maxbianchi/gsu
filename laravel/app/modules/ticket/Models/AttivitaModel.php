...
EOF;
    ...
    public function getSingoleAttivita($idattivita){
        $sql = "SELECT S.ID,S.IDATTIVITA,S.DESCRIZIONE, S.TEMPO,S.INCARICOA,CONVERT(VARCHAR(10),S.INSERITOIL,105 ) INSERITOIL, CONVERT(VARCHAR(10),S.INSERITOIL,108 ) INSERITOIL_ORA, T3.DESCRIZIONE INCARICOA_ATTIVITA FROM TICKET.dbo.SINGOLE_ATTIVITA S LEFT JOIN TICKET.dbo.TECNICI T3 ON S.incaricoa = T3.IDTECNICO  WHERE IDATTIVITA='$idattivita' ORDER BY ID";
        $request = DB::select($sql);
        return $request;
    }
